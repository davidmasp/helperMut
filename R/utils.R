...
empirical_sampling_posterior_CI <- function(x,
                                            px,
                                            times = 1000,
                                            ci = 0.95,
                                            seed = NULL) {
  # adapted from here https://stats.stackexchange.com/a/240777/145935
  
  if(!is.null(seed)){set.seed(seed)}
  
  # sample from the "empirical" distribution
  samp <- sample(x, times, replace = TRUE, prob = px)
  
  # and take sample quantiles
  lowerV = (1 - ci)/2
  upperV = 1 - lowerV
  quantile(samp, c(lowerV, upperV))
  
}


numerical_posterior_CI <- function(x,px,ci = 0.95){
  # adapted from here https://stats.stackexchange.com/a/240777/145935
  # I think this only works if the posterior is symetric-ish
  cpxx <- cumsum(px) / sum(px)
  ...
  res[1] = x[which(cpxx >= lowerV)[1]]   # lower boundary
  res[2] = x[which(cpxx >= upperV)[1]-1] # upper boundary
  
  names(res) = c(scales::percent(lowerV),scales::percent(upperV))
